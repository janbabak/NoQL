  ...
  async function queryChat(): Promise<void> {
    ...
    try {
      ...
      if (chat?.name == NEW_CHAT_NAME) {
        const updatedName: string = response.data.messageWithResponse.message.length < CHAT_NAME_MAX_LENGTH
          ? response.data.messageWithResponse.message
          : response.data.messageWithResponse.message.substring(0, CHAT_NAME_MAX_LENGTH)
        ...
        const newChatHistory: ChatDto[] = chatHistory
